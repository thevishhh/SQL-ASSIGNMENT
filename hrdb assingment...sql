...
select first_name, last_name, department_id, salary  from employees where commission_pct >0 order by salary desc;
...
select last_name, job_id, salary from employees where job_id = 'SA_REP' or job_id= 'ST_CLERK' && salary != 2500 or 3500 or  5000; 

                                           ---------------- ASSIGNMENT 2 ---------------------

--- 1) Display the maximum, minimum and average salary and commission earned.
select salary, commission_pct from employees where salary= (select max(salary)from employees) or commission_pct= (select max(commission_pct)from employees);

select salary, commission_pct from employees where salary= (select min(salary)from employees) or commission_pct= (select min(commission_pct)from employees);
...
select department_id, department_name, sum(employee_id) from emp_details_view group by department_name;
...
select concat(first_name," ", last_name)as fullname, commission_pct from employees  where commission_pct like "%0%";
...
select salary, department_id from employees where salary= (select max(salary)from employees) order by department_id; 

--- 7) Display the 'employee's name, salary and commission multiplied by 2. If an Employee doesnt earn the commission, then display as 'No commission'. Name the columns appropriately
 select  first_name, last_name , salary,  2*commission_pct,
 case
   when commission_pct is null then 'NO COMMISSION'
   else 2*commission_pct
   end as '2*commission_pct'
from employees;
...
select sum(salary) ,manager_id,first_name,last_name from employees group by manager_id,first_name,last_name;
...
select department_id, salary, concat(first_name," ", last_name)as fullname, manager_id from employees  where last_name like "%_a%"  order by manager_id;
...
from employees;
 ...
 select substring(upper(lower(last_name)) , 2, 5) from employees;

 
 
--- 2. Write a query that displays the 'employee's first name and last name along with "a " in between for e.g.: first name : Ram; last name : Kumar then Ram-Kumar. Also displays the month on which the employee has joined.2. Write a query that displays the employees first name and last name along with "a " in between for e.g.: first name : Ram; last name : Kumar then Ram-Kumar. Also displays the month on which the employee has joined.
...
SELECT 
    CONCAT(
        ...
        '00E') as modified_employee_id,department_id,salary from employees;
...
SELECT CONCAT(LOWER(SUBSTRING(first_name, 1, 1)), LOWER(last_name), '@systechusa.com') AS "e-mail address", first_name 
FROM employees;
...
join locations
on departments.department_id = locations.location_id
...
JOIN departments d
 ...
 --- 10. Display the names and hire date for all employees who were hired before their managers along withe their manager names and hire date. Label the columns as Employee name, emp_hire_date,manager name,man_hire_date
  ...
  hire_date as employee_hire_date from employees;
...
---  2. Create a query to display the employee numbers and last names of all employees who earn more than the average salary. Sort the results in ascending order of salary.
  ...
  where salary > 6500 order by salary;	
...
---- 6. Display the department number, last name, and job ID for every employee in the OPERATIONS department.
  ...
  --- 7. Modify the above query to display the employee numbers, last names, and salaries of all employees who earn more than the average salary and who work in a department with any employee with a 'u'in their name.
   select employee_id, last_name , salary from employees
  where salary > 6500 and last_name like '%u%';	
        ...
        when department_id in (10.30) then (salary*5/100) + salary 
            when department_id = 20 then (salary * 10/100) + salary 
             when department_id in (40,50) then (salary * 15/100) + salary 
              when department_id =60 then "NOT GIVEN RAISE" end as raised_percent_salary from employees;
